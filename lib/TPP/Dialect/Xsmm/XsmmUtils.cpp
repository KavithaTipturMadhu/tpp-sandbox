...
createFlatListOfOperandStaticDims(vector::ContractionOp contractOp) {
  ...
  for (auto op = 0; op < contractOp.getOperation()->getNumOperands(); op++) {
...
LogicalResult checkAccess(PatternRewriter &rewriter,
                          ...
                          bool checkForTransposes, int operandIndex) {
  ...
  auto checkStridesAndGetLdaAndBatch =
      [&](Value operand, AffineMap map, int minorDim, int majorDim,
  ...
  auto ldaVal =
      checkStridesAndGetLdaAndBatch(operandA, indexingMap[opZeroIndex], k, m, 0,
  ...
  auto ldbVal = checkStridesAndGetLdaAndBatch(
      operandB, indexingMap[opOneIndex], n, k, 1, batchPos, isVnni, vnniFactor);
  ...
  // C(m, n)
  auto ldcVal = checkStridesAndGetLdaAndBatch(operandC, indexingMap[2], n, m, 2,
                                              batchPos, isVnni, vnniFactor);
  if (failed(ldcVal)) {
