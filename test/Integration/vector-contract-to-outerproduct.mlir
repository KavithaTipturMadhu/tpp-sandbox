...
// RUN: tpp-run %s --contract-to-outer-product -e entry --entry-point-result=void -seed 123 -print > %t.2
// RUN: diff %t.1 %t.2 | FileCheck %s --check-prefix=DIFF --allow-empty

// RUN: tpp-run %s -e permA --entry-point-result=void -seed 123 -print > %t.1
// RUN: tpp-run %s -e permA --contract-to-outer-product --entry-point-result=void -seed 123 -print > %t.2
// RUN: diff %t.1 %t.2 | FileCheck %s --check-prefix=DIFF-PERMA --allow-empty

// RUN: tpp-run %s  -e permB --entry-point-result=void -seed 123 -print > %t.1
// RUN: tpp-run %s -e permB --contract-to-outer-product --entry-point-result=void -seed 123 -print > %t.2
// RUN: diff %t.1 %t.2 | FileCheck %s --check-prefix=DIFF-PERMA --allow-empty

// RUN: tpp-run %s -e permAB --entry-point-result=void -seed 123 -print > %t.1
// RUN: tpp-run %s -e permAB --contract-to-outer-product --entry-point-result=void -seed 123 -print > %t.2
